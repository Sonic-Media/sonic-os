...
const MANAGEMENT_DASHBOARD_REFRESH_MS = 12_000;

export function useManagementDashboardRefresh(): void {
  ...
  const refreshAll = useCallback(async () => {
    ...
    try {
      ...
      console.error("[management-dashboard] live refresh failed:", error);
    } finally {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(
      refreshIfVisible,
      MANAGEMENT_DASHBOARD_REFRESH_MS
    );
